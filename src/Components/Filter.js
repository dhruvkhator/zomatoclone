...
class Filter extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            method: 'POST',
            url: 'https://zomato-clone-backend27.herokuapp.com/filter',
        ...
        axios({
            method: 'GET',
            url: 'https://zomato-clone-backend27.herokuapp.com/locations',
            headers: { 'Content-Type': 'application/json' }
        })
            .then(response => {
    ...
    handlePageChange = (page) => {
        ...
        axios({
            ...
            .then(response => {
                this.setState({ restaurants: response.data.restaurants, page, currentPage: page, pageCount: response.data.pageCount })
            })
            .catch(err => {
                console.log(err);
            })
    ...
    handleCuisineChange = (cuisineId) => {
        const { mealtype, cuisine, location, sort, page, lcost, hcost } = this.state;
    ...
    render() {
        const { restaurants, locations, pageCount, currentPage } = this.state;
        return (
            <div>
                <div>
                        ...
                        <div className="row">
                            <div className="col-sm-4 col-md-4 col-lg-4 filter-options">
                                ...
                                <div id="filter" className="collapse show">
                                    ...
                                    <div>
                                        <input type="checkbox" name="cuisine" value="1" onChange={() => this.handleCuisineChange(1)} />
                                        <span className="checkbox-items">North Indian</span>
                                    </div>
                                    <div style={{ display: "block" }} >
                                        <input type="checkbox" name="cuisine" value="2" onChange={() => this.handleCuisineChange(2)}/>
                                        <span className="checkbox-items">South Indian</span>
                                    </div>
                                    <div>
                                        <input type="checkbox" name="cuisine" value="3" onChange={() => this.handleCuisineChange(3)} />
                                        <span className="checkbox-items">Chineese</span>
                                    </div>
                                    <div>
                                        <input type="checkbox" name="cuisine" value="4" onChange={() => this.handleCuisineChange(4)}/>
                                        <span className="checkbox-items">Fast Food</span>
                                    </div>
                                    <div>
                                        <input type="checkbox" name="cuisine" value="5" onChange={() => this.handleCuisineChange(5)}/>
                            ...
                            <div className="col-sm-8 col-md-8 col-lg-8">
                                ...
                                {restaurants.length > 0 ?
                                    <div className="pagination">
                                        <span className="page-num" onClick={() => this.handlePageChange(currentPage - 1)}>&laquo;</span>
                                        {pageCount.map((pageNo) => {
                                            return <span className="page-num" onClick={() => this.handlePageChange(pageNo)}>{pageNo}</span>
                                        })}
                                        < span className="page-num" onClick={() => this.handlePageChange(currentPage + 1)} >&raquo;</span>
                                    </div> : null}
                            </div>
                        </div>
                    </div>
